...
const LeadFormModal = ({ lead, onSave, onClose, usersList }) => {
    ...
    const inputCls = 'w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-purple-500 text-slate-800 dark:text-white';
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 animate-in fade-in duration-150">
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                {/* Footer */}
                <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50">
                    <button onClick={onClose} className="px-3 py-1.5 text-xs font-bold text-slate-500 hover:text-slate-800 dark:hover:text-white uppercase tracking-wider">
                        Discard
                    </button>
                    <button
                        onClick={handleSave}
                        disabled={saving || (!form.companyName.trim() && !form.contactName.trim())}
                        className="flex items-center gap-1.5 px-4 py-1.5 bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold rounded-lg transition-all disabled:opacity-50 uppercase tracking-wider"
                    >
            ...
            {stagesToShow.map(stage => {
                ...
                return (
                    <div key={stage} className="flex-shrink-0 w-64">
                        <div className={`flex items-center justify-between px-3 py-2 rounded-t-lg border-b-2 ${c.bg} border-current/20`}>
                            <div className={`flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider ${c.text}`}>
                                <span className={`w-1.5 h-1.5 rounded-full ${c.dot}`} />
                                {stage}
                                <span className="ml-1.5 px-1.5 py-0.5 rounded-full bg-slate-100 dark:bg-black/20 text-[9px]">
                                    {stageLeads.length}
                                </span>
                            </div>
                        </div>

                        <div className="space-y-2 pt-2 min-h-[300px] bg-slate-50 dark:bg-slate-900/40 rounded-b-lg p-2 border border-slate-200 dark:border-slate-800 border-t-0">
                            {stageLeads.map(lead => (
                                <div key={lead.id} onClick={() => onEdit(lead)}
                                    className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-3 cursor-pointer hover:border-purple-400 transition-all shadow-sm">
                                    <h4 className="text-xs font-bold text-slate-800 dark:text-white truncate tracking-tight">{lead.companyName || lead.contactName}</h4>
                                    <p className="text-[10px] text-slate-400 truncate mt-0.5 line-clamp-1">{lead.contactName || lead.location || '—'}</p>
                                    
                                    <div className="flex items-center justify-between mt-3">
                                        <div className="flex items-center gap-1 text-[9px] text-slate-400 font-bold uppercase">
                                            <MapPin size={9} />{lead.location || 'Local'}
                                        </div>
                                        {lead.assignedTo && (
                            ...
                            {stageLeads.length === 0 && (
                                <div className="h-20 border-2 border-dashed border-slate-200/50 dark:border-slate-800/50 rounded-lg flex flex-col items-center justify-center text-[9px] text-slate-300 uppercase font-bold tracking-widest gap-1">
                                    Empty
                                </div>
                            )}
                        </div>
                ...
                <div className="flex items-center justify-between gap-4 mb-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => { setEditingLead(null); setShowForm(true); setShowDashboard(false); }}
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all shadow-sm">
                ...
                <>
                    ...
                    {viewMode === 'analytics' ? (
                        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm space-y-4">
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pb-4 border-b border-slate-100 dark:border-slate-700">
                                {[
                        ...
                        /* Only show very compact stats bar in other views */
                        <div className="flex items-center gap-6 py-2 border-b border-slate-200 dark:border-slate-700 mb-4 px-1 overflow-x-auto whitespace-nowrap scrollbar-hide">
                            {[
                                { label: 'Total Leads', value: analytics.total, color: 'text-slate-600' },
                                { label: 'In Pipeline', value: analytics.pipeline, color: 'text-blue-600' },
                            ...
                            <div className="flex items-center gap-2 mb-3">
                                <div className="relative flex-1 max-w-sm">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-8 pr-3 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-purple-500 transition-all"
                                        ...
                                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                                            ...
                                            {filteredLeads.map((lead, idx) => (
                                                <tr key={lead.id} onClick={() => { setEditingLead(lead); setShowDashboard(true); }}
                                                    className={`group cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors ${idx % 2 === 0 ? '' : 'bg-slate-50/30 dark:bg-slate-800/20'}`}>
                                                    <td className="px-3 py-1.5 min-w-[140px]">
                                                        <div className="flex items-center gap-2">
                                                            <div className="w-5 h-5 rounded bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-400 flex items-center justify-center font-bold text-[10px]">
                                                                {(lead.companyName || lead.contactName || '?')[0].toUpperCase()}
                                                            </div>
                                                            <div className="min-w-0">
                                                                <p className="text-xs font-bold text-slate-800 dark:text-white truncate">{lead.companyName || '—'}</p>
                                                                {lead.location && <p className="text-[9px] text-slate-400 truncate mt-0.5">{lead.location}</p>}
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="px-3 py-1.5 hidden md:table-cell">
                                                        <p className="text-xs text-slate-700 dark:text-slate-300 truncate">{lead.contactName || '—'}</p>
                                                    </td>
                                                    ...
                                                    <td className="px-3 py-1.5">
                                                        <div className="relative inline-block w-full max-w-[100px]" onClick={e => e.stopPropagation()}>
                                                            <select
                                                                ...
                                                                className={`appearance-none cursor-pointer w-full pl-2 pr-6 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider focus:outline-none border shadow-sm ${STAGE_COLORS[lead.stage]?.bg} ${STAGE_COLORS[lead.stage]?.text} border-current/10`}
                                                            >
                                                                {STAGES.map(s => <option key={s} value={s}>{s}</option>)}
                                                            </select>
                                                            <ChevronDown size={10} className={`absolute right-1.5 top-1/2 -translate-y-1/2 pointer-events-none opacity-40 ${STAGE_COLORS[lead.stage]?.text}`} />
                                                        </div>
                                                    </td>
                                                    <td className="px-3 py-1.5 hidden xl:table-cell text-[10px] text-slate-500 uppercase tracking-wider">
                                                        {lead.source || '—'}
                                                    </td>
                                                    <td className="px-3 py-1.5 hidden md:table-cell text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-tighter">
                                                        {lead.assignedTo || '—'}
                                                    </td>
                                                        ...
                                                        <div className="flex items-center justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                                                            {(isAdmin || lead.assignedTo === user.username || lead.createdBy === user.username) && (
                                                                <button
                                                                    ...
                                                                    className="p-1.5 rounded-lg text-slate-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/40 transition-all"
                                                                    title="Delete">
                                                                    <Trash2 size={12} />
                                                                </button>
                                                            )}
