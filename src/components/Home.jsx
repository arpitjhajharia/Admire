...
const Home = ({ onSelectModule, darkMode, perms }) => {
    ...
    const modules = allModules.filter(mod => perms?.[mod.permKey]);
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
                {modules.map((mod) => (
                    <button
                        ...
                        className={`flex flex-col items-start p-3 sm:p-6 rounded-xl sm:rounded-2xl border transition-all duration-300 text-left hover:-translate-y-1 hover:shadow-xl cursor-pointer ${mod.borderColor} bg-white dark:bg-slate-800`}
                    >
